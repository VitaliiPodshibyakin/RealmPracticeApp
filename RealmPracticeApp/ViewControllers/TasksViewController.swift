...
class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let taskList = indexPath.section == 0 ? currentTasks[indexPath.row] : completedTasks[indexPath.row]
        ...
        let doneAction = UIContextualAction(style: .normal, title: doneTitle) { _, _, isDone in
            StorageManager.shared.doneTask(taskList)
        ...
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") { _, _, _ in
            StorageManager.shared.deleteTask(taskList)
...
extension TasksViewController {
    ...
    private func showAlert(with task: Task? = nil, completion: (() -> Void)? = nil) {
        let alert = AlertController.createAlert(withTitle: "Task", andMessage: "What do you want to do?")
        alert.actionForTask(with: task) { newName, newNote  in
            if let task = task, let completion = completion {
                StorageManager.shared.editTask(task, newName: newName, newNote: newNote)
                completion()
            } else {
